...
| `card-classify.js` | 188 | `MtgCardClassify` | The one copy of "what does this card do", read off rules text. Shared with `graph/ingest/02-build-csv.mjs` so a card typed on the graph is read by the rules the corpus was baked with | graph |
...
| `data/graph.json` | 7.0 MB | 7,764 cards with rules-derived edges, EDHREC co-play (10,315 `playedWith` pairs), prices, ownership, facets, and the 6 deck overlays | `graph/ingest/07-export-app.mjs --commanders` | `graph-page.js` |
