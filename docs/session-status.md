...
| #205 | Play-space plan (Rob, 14 September): `docs/crankmagic-playspace-plan.md` — the Table reworked into a sandbox where moves are proposals until Confirm, three zones (source shelves, play space, destination band), a draw pile and 1–4 trays, generated canvases, free pile placement, glossary hovers; with a critical assessment of the brief that changes six of its instructions (what "clear all statuses" can mean per row kind, Watched vs owning a card, Wanted = Reserved, the click/double-click conflict, 3 columns a side eating the middle, staging moves but not field edits) and a performance and motion budget — speed is the requirement (the board is built once and thereafter mutated rather than rebuilt on every draw, ~20 pictures at rest, 60 fps drags, a drop under 16 ms), motion and extra visuals optional. Revised the same day with Rob's answers (§6): the middle holds; **Watched is redefined to cover a card you own** — *filed in that deck's collection group*, which `readiness()` already counted and had never named, so no new field and nothing in the live library changes status; the trays mean Reserved and confirming one adds the card to the deck's list as well; arrows under the draw pile, one click lays a pile out, a back arrow at every level, no double click; the sandbox is one overlay all three lenses read through so they cannot diverge; a sitting persists in localStorage and is re-validated against the revision it was staged at; Confirm is per table; and the table has two jobs — calibrate a deck, or sort the shelf into collection groups, which needs a way to search cards in from the catalog and is its own PR | A |
