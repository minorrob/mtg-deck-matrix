...
## 9. Decided, 15 September

All five answered. Four went to the recommendation; two did not, and both changes are for the
better.

| Question | Decided | What it means |
|---|---|---|
| **The key** | **Bring your own, in the browser** | The key lives in this browser's `localStorage` and the page calls the API directly. Playable soonest, single-player, and nothing about it blocks the Worker proxy the day someone else wants to sit down. |
| **Rules fidelity** | **Kitchen table now, stricter later** | The referee is the simplified stack of §8 — priority only when something is on it and a seat holds an instant or flash permanent — but the rules are written so a stricter layer could replace them piece by piece rather than requiring a rewrite. That is a design constraint on G3a, not a second engine: no rule may be spelled inline in the turn loop; each is a named, replaceable step the loop calls. |
| **Spend** | **A per-game cap, plus the meter** | A ceiling per game. When it is reached every seat falls back to the deterministic pilot (§5.3) and the game still finishes — it just stops spending. The meter shows what each game cost either way. |
| **Does a game write back?** | **Logged, and marked as an AI game** | It joins the deck's game record beside the games played at a real table, badged. The simulator wants the data and a real record is worth more than a clean one; the badge means a win rate can still be read either way. |
| **Deck generation** | **To a budget *and* the bracket** | *Not the recommendation.* A generated opponent is built to two constraints rather than matched to your measured score. Matched-to-score stays available in the dropdown. |

**Why "budget and bracket" is the better answer.** Matching a generated deck to your measured
score produces a sparring partner, not an opponent: it is built *around* you, so beating it says
nothing except that the generator aimed well. Both of these are constraints the real world
actually imposes, and a deck built under them is a deck someone could really have brought — so the
...
- **The bracket is the table's** and binds every seat alike — it is what caps the Game Changers,
  and a generated deck has no more right to ignore it than an imported one does. It is not a field
  on the seat form; it is the bracket at the top of the lobby, and the form says which one it will
  build to and how many Game Changers that allows.
- **The budget is the seat's own**, because what an opponent spent is a fact about that opponent
  rather than a rule of the table. It defaults to the house's `deckCap` ($225 in
  `crankmagic-rules.js`), takes the same `perCardMax` ceiling the Shop reads, and is editable per
  seat — so a pod can hold a $60 precon, a $225 build and a $600 one and mean it.

The seat carries both in its name — *Krenko, Mob Boss (generated · bracket 3 · $225)* — so nobody
