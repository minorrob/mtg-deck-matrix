...
Use Rob's supplied floating-cube mat image for the human seat, retaining its recognizable artwork and zone placement. Other seats use a restrained dark mat with the same geometry. Table overview shows four mats around the shared counter. **Focus board** fills the viewport with wrapping groups for creatures, artifacts, enchantments, planeswalkers, battles, other permanents, and mana / lands; individual card inspection enlarges artwork further. Avoid fixed-height blank space and a single horizontal card strip. Give Focus a card-size control (prototype: 120–260 px), persistent counter / public-zone access, and an accessible own-hand section. Assign multi-type permanents once; collapse empty categories. The prototype uses recorded card-facts types, with mana rocks under Artifacts; the playable engine must supply current types and attachment relationships. Libraries display backs and counts, never an unrevealed top card. Public piles open their full visible contents. Mat life displays mirror the central counter's engine state.
