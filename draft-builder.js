...
(function(root,factory){const api=factory();if(typeof module==='object'&&module.exports)module.exports=api;root.CrankDraft=api;})(globalThis,function(){
  ...
  function build({commanders,cards,definition,available={},benchOnly=false,pinned=[]}){
    ...
    commanders.forEach(c=>add(c,1,true));pinned.forEach(r=>{const c=all.get(r.cardId);if(c)add(c,r.quantity,true);else issues.push('A pinned card is missing from the catalog.');});
    ...
    if(count()!==100)issues.push(`Only ${count()} of 100 cards could be chosen within the catalog, eligible-copy and price limits. No limit was relaxed.`);
