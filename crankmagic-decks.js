...
/* ONE CARD, TWO COLUMNS: where the deck stands, and what it costs. The readiness figures and
   ...
   thing twice. Progress is the left column -- in box, to pull, ordered, to buy, Game Changers,
...
function stats(d){const r=M.readiness(C.state,d),R=globalThis.CrankRules,cap=d.definition.budget??(R?R.RULES.deckCap:null),perCard=d.definition.perCardCap??(R?R.RULES.perCardMax:null);
  ...
  return `<section class="cm-deck-summary${tone}" aria-label="Deck progress and cost"><div class="cm-summary-col"><h3>Progress</h3><div class="cm-summary-figures">${fig(r.sleeved,'In physical deck')}${r.standIns||r.remove?fig(r.standIns,`of them substitute${r.standIns===1?'':'s'}${r.remove?` · ${r.remove} to take out`:''}`):''}${fig(pullCount(r),'Ready to add')}${fig(r.ordered,'Ordered')}${fig(r.toBuy,d.status==='draft'?'Not yet reserved':'To buy')}${fig(`${gc} / ${GC_LIMIT}`,'Game Changers')}</div>${C.readinessBar(r)}${C.readinessLegend(r)}</div><div class="cm-summary-col cm-summary-cost"><h3>Cost</h3><div class="cm-summary-figures">${fig(C.money(r.costToFinish),'$ to finish')}${fig(C.money(r.paid),'Paid so far')}${fig(C.money(r.marketValue),`Market value${pct!==null?` · ${Math.round(pct)}% of cap`:''}`)}${fig(cap===null?'—':C.money(cap),'Cap')}${fig(overCap,`line${overCap===1?'':'s'} over the 110% cap`,overCap?'cm-amber':'')}${dear?fig(dear,`card${dear===1?'':'s'} over ${C.money(perCard)}`,'cm-amber'):''}</div>${cap>0?`<div class="cm-budget-bar" role="img" aria-label="Market value ${Math.round(pct)}% of the cap"><i style="width:${Math.min(100,pct)}%"></i></div>`:''}</div></section>`;}
...
const toGo=r.target-r.owned,badge=d.archived?['draft','Archived']:d.status==='draft'?['draft','Defining']:r.complete?['inbox','Complete']:r.playable?['standin',`Playable · ${r.standIns} substitute${r.standIns===1?'':'s'} · ${toGo} to go`]:['buy',`Building · ${toGo} to go`];
const stage=d.archived?'archived':d.status==='draft'?'defining':r.complete?'complete':r.playable?'playable':'building';
return `<article class="cm-deck-tile cm-stage-${stage}${picked?' is-picked':''}">${art(d)?`<img class="cm-deck-art" src="${e(art(d))}" alt="" loading="lazy">`:initials(d)}<label class="cm-tile-pick" title="Tick to compare this deck"><input type="checkbox" data-action="compare-pick" data-deck="${e(d.id)}"${picked?' checked':''} aria-label="Compare ${e(d.name)}"></label><button data-action="deck" data-deck="${e(d.id)}"><small>${e(d.name)}</small><h3>${e(commander(d)||'Choose a commander')}</h3>${(()=>{const m=mechanicsOf(d);return m.list.length?`<p${m.derived?' class="cm-tile-derived" title="Read from the list. Name your own in Deck Definition."':''}>${e(m.list.join(' · '))}</p>`:'';})()}<span class="cm-tile-mana">${C.colors(C.state.cards[d.commanders[0]]?.colorIdentity)}</span>${C.pill(e(badge[1]),badge[0],'title="Cards not yet owned"')}</button><div class="cm-tile-ready">${C.readinessBar(r)}<span class="cm-tile-caption">${r.sleeved} in deck${r.standIns?` (${r.standIns} substitute${r.standIns===1?'':'s'})`:''} · ${pullCount(r)} to add · ${r.toBuy} buy${(()=>{const g=C.state.games.filter(x=>x.deckId===d.id);if(!g.length)return '';const w=g.filter(x=>x.outcome==='win').length,l=g.filter(x=>x.outcome==='loss').length;return ` · ${w}–${l}`;})()}</span></div><footer><span>B${d.definition.baseBracket}${scoreOf(latestReport(d))?` · <span class="cm-badge" title="Latest measured score">${e(scoreOf(latestReport(d)))} pts</span>`:''}</span><span class="cm-muted">${d.locked?'Locked':''}</span><span class="cm-tile-tools"><button type="button" class="cm-icon-button cm-tile-menu-btn" data-action="deck-menu" data-deck="${e(d.id)}" aria-haspopup="menu" aria-label="Deck options for ${e(d.name)}">⋯</button></span></footer></article>`;}).join('')}</div>`:`<section class="v-panel cm-empty"><h2>A fresh library. A new deck.</h2><p>Start from a commander and let the Lab build the 99, or paste a list you already have. Adding a plan creates no owned copies.</p><div class="cm-actions">${b('Select a commander','open-lab',{},true)}${b('Import a list','import-list')}${b('Import a backup','restore')}</div></section>`);$('#cm-show-archived').addEventListener('change',ev=>{showArchived=ev.target.checked;C.render();});};
...
const rowActions=d.archived?b('Restore as draft','restore-deck',{deck:d.id})+b('Delete permanently','delete-deck',{deck:d.id}):(d.status==='final'?b(pullCount(ready)+ready.remove>0?`Pull sheet (${pullCount(ready)+ready.remove})`:'Pull sheet','deck-pull',{deck:d.id}):b('Finalize & reserve','finalize',{deck:d.id},true))+b(`Buy list (${ready.toBuy})`,'deck-buy-list',{deck:d.id})+b(`Upgrades (${d.slots.filter(r=>r.purpose!=='main').length})`,'deck-upgrades',{deck:d.id})+b('Log a game','log-game',{deck:d.id})+b('Card status','deck-status-menu',{deck:d.id},false,{caret:'down'})+b('More','deck-more-menu',{deck:d.id},false,{caret:'down'});
C.main.innerHTML=`<section class="cm-deck-hero"${heroArt?` style="--hero:url('${e(heroArt)}')"`:''}><div class="cm-deck-hero-copy"><div class="v-eyebrow cm-eyebrow-warm">My Decks / Deck overview</div><h1>${e(d.name)}</h1><p>${e(commander(d))} ${C.colors(C.state.cards[d.commanders[0]]?.colorIdentity)} <span class="cm-badge ${ready.ready?'good':''}">${d.archived?'Archived':d.status==='draft'?'Defining':ready.complete?'Complete':ready.playable?'Playable':'Building'}</span> <span class="cm-badge">Bracket ${e(String(d.definition.baseBracket))}–${e(String(d.definition.bracketCeiling))}</span>${overCap?` <span class="cm-badge warn" title="Recorded prices of the main list against the definition’s total cap">Over the ${e(C.money(cap))} cap · about ${e(C.money(spend))}</span>`:''}${latest?` <span class="cm-badge" title="Latest measured score">Measured ${e(scoreOf(latest))} pts</span>`:''}${attached(d)?` <span class="cm-badge">Group: ${e(attached(d).name)}</span> <button type="button" class="cm-text-button cm-hero-link" data-action="deck-group" data-group="${e(d.groupId)}">Open group</button>`:''}${d.locked?' <span class="cm-badge warn">Locked</span>':''}</p></div><div class="cm-actions cm-deck-actions">${b('View deck cards','deck-cards',{deck:d.id},true)}${rowActions}${d.status==='draft'?b('Edit card list','edit-list',{deck:d.id}):''}</div></section>`+stats(d)+/* THREE THINGS YOU DO TO A DECK, AND TWO DRAWERS FOR THE REST. Nine buttons in one row
...
/* The action row sits in the hero beside View deck cards (rowActions above): pull what you
...
actions['deck-status-menu']=el=>{const d=M.deck(C.state,el.dataset.deck),ladder=C.statusLadder||[];
