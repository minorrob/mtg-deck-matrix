...
const rowActions=d.archived?b('Restore as draft','restore-deck',{deck:d.id})+b('Delete permanently','delete-deck',{deck:d.id}):(d.status==='final'?b(pullCount(ready)+ready.remove>0?`Ready to add (${pullCount(ready)+ready.remove})`:'Ready to add','deck-pull',{deck:d.id}):b('Finalize & reserve','finalize',{deck:d.id},true))+b(`Buy list (${ready.toBuy})`,'deck-buy-list',{deck:d.id})+b(`Upgrades (${d.slots.filter(r=>r.purpose!=='main').length})`,'deck-upgrades',{deck:d.id})+b('Log a game','log-game',{deck:d.id})+b('Card status','deck-status-menu',{deck:d.id},false,{caret:'down'})+b('More','deck-more-menu',{deck:d.id},false,{caret:'down'});
