...
const body={
  overview:()=>(d.status==='draft'&&overCap?note(`Over the ${C.money(cap)} cap by about ${C.money(spend-cap)} at recorded prices. Finalize offers to raise the cap or trim the list.`,true):'')+stats(d)+`<p class="cm-deck-next" id="cm-deck-next"><strong>Next:</strong> ${e(nextLine(d,ready))}</p>`,
...
function cardsTab(d,cards,curve,max,types){
  ...
  for(const slot of d.slots.filter(r=>r.purpose==='main')){const c=C.state.cards[slot.cardId];if(!c)continue;groups.get(d.commanders.includes(slot.cardId)?'Commander':ORDER.find(k=>k!=='Commander'&&k!=='Other'&&c.typeLine.includes(k))||'Other').push({slot,c});}
