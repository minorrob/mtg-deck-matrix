...
C.HELP.decks={title:'My Decks',body:`<p>Every deck you have, at its stage: <strong>Defining</strong> while the list is being written, <strong>Building</strong> while cards are still to buy or to add, <strong>Playable</strong> when substitutes make up the hundred, <strong>Complete</strong> when every reserved copy is in the physical deck.</p><p>Each tile's bar is the hundred: in the physical deck, ready to add, ordered, to buy. Tick two or more tiles to compare them; the ⋯ on a tile archives or restores it.</p>`};
C.HELP.deck={title:'A deck page',body:`<p>The header is the summary: commander, stage, bracket and the latest measured score. <strong>Progress</strong> counts the physical deck, ready to add, ordered and to buy; <strong>Cost</strong> is what finishing costs against the cap.</p><p>A <strong>finalized</strong> deck's reservations track what its list needs: Ready to add walks the cards you own into the physical deck, and the buy list is what is left. A <strong>draft</strong> has no reservations until you finalize it. An <strong>archived</strong> deck keeps its history; its copies were released, and where each one physically is stays recorded.</p><p>Composition counts a multitype card under each of its types; Ramp is read from the rules text by the same classifier the graph uses. SWOT is structural — text matches, not modeled availability; measured performance is under Simulation history. Upgrades, the card status ladder and the deck's settings are under More.</p>`};
...
function mechanicsOf(d){if(d.definition.mechanics.length)return {list:d.definition.mechanics,derived:false};const main=d.slots.filter(r=>r.purpose==='main').map(r=>C.state.cards[r.cardId]).filter(Boolean);const list=CrankCatalog.deckMechanics?CrankCatalog.deckMechanics(main,C.state.cards[d.commanders[0]]):[];return {list,derived:true};}
...
function workingHTML(d){
  ...
  const planned=g?[...g.entries.map(r=>({name:C.state.cards[r.cardId]?.name||'',quantity:r.quantity,why:r.notes||'',status:'Planned'})),...C.state.lots.filter(l=>l.groupIds.includes(g.id)&&!(l.allocation&&l.allocation.deckId===d.id)&&!inList.has(l.cardId)).map(l=>({name:C.state.cards[l.cardId]?.name||'',quantity:l.quantity,why:l.notes.replace(/^D\d+:\s*/,''),status:C.source(l.source)}))]:[];
  ...
  return `<section class="v-panel cm-working" id="cm-sec-working"><h2>Working list</h2><div class="cm-grid-2"><div><h3>Options · first to swap out <small>${options.length}</small></h3>${options.length?`<ul class="cm-working-list">${options.map(r=>item(C.state.cards[r.cardId]?.name||'',r.quantity,'Option',r.optionWhy,b('Clear','flag-slot',{deck:d.id,slot:r.id}))).join('')}</ul>`:'<p class="cm-muted">None flagged. Flag a card from its row in the Collection: Actions → Flag as option.</p>'}${pins?`<p class="cm-muted">${pins} pinned card${pins===1?'':'s'} stay whatever happens.</p>`:''}</div><div><h3>Coming in <small>${planned.length+suggestions}</small></h3>${planned.length?`<ul class="cm-working-list">${planned.map(p=>item(p.name,p.quantity,p.status,p.why)).join('')}</ul>`:''}${suggestions?`<p>${suggestions} recommended update${suggestions===1?'':'s'} from the Upgrade Path, each attached to the card it comes in for. ${b('See them','deck-upgrades',{deck:d.id})}</p>`:''}${!planned.length&&!suggestions?'<p class="cm-muted">Nothing planned. Add a planned card to the deck’s group from the Collection.</p>':''}</div></div></section>`;
}
...
function stats(d){const r=M.readiness(C.state,d),R=globalThis.CrankRules,cap=d.definition.budget??(R?R.RULES.deckCap:null),perCard=d.definition.perCardCap??(R?R.RULES.perCardMax:null);
  ...
  const fig=(v,label,cls='')=>`<div><strong${cls?` class="${cls}"`:''}>${v}</strong><span>${label}</span></div>`;
  return `<section class="cm-deck-summary${tone}" aria-label="Deck progress and cost"><div class="cm-summary-col"><h3>Progress</h3><div class="cm-summary-figures">${fig(r.sleeved,'Physical deck')}${r.standIns||r.remove?fig(r.standIns,`of them substitute${r.standIns===1?'':'s'}${r.remove?` · ${r.remove} to take out`:''}`):''}${fig(pullCount(r),'Ready to add')}${fig(r.ordered,'Ordered')}${fig(r.toBuy,d.status==='draft'?'Not yet reserved':'To buy')}${fig(`${gc} / ${GC_LIMIT}`,'Game Changers')}</div>${C.readinessBar(r)}</div><div class="cm-summary-col cm-summary-cost"><h3>Cost</h3><div class="cm-summary-figures">${fig(C.money(r.costToFinish),'$ to finish')}${fig(C.money(r.paid),'Paid so far')}${fig(C.money(r.marketValue),`Market value${pct!==null?` · ${Math.round(pct)}% of cap`:''}`)}${fig(cap===null?'—':C.money(cap),'Cap')}${fig(overCap,`line${overCap===1?'':'s'} over the 110% cap`,overCap?'cm-amber':'')}${dear?fig(dear,`card${dear===1?'':'s'} over ${C.money(perCard)}`,'cm-amber'):''}</div>${cap>0?`<div class="cm-budget-bar" role="img" aria-label="Market value ${Math.round(pct)}% of the cap"><i style="width:${Math.min(100,pct)}%"></i></div>`:''}</div></section>`;}
...
views.decks=async params=>{const did=params.get('deck');if(did){const found=C.state.decks.find(x=>x.id===did);if(!found){C.main.innerHTML=C.pageHead('My Decks',b('My Decks','home',{},true))+note('Deck not found: this library has no deck with that id. It may live in another browser’s library, or under a different link.',true);return;}await overview(found);return;}const decks=C.state.decks.filter(d=>showArchived||!d.archived),picks=(C.state.preferences.comparisonPicks||[]).filter(id=>C.state.decks.some(d=>d.id===id));
...
C.main.innerHTML=(anyDecks?C.pageHead('My Decks',b('Create a deck','new-deck',{},true)+b('Open Deck Lab','open-lab')+compare,'decks')+`<div class="cm-toolbar cm-toolbar-split">${howLink}<label class="cm-checkbox cm-show-archived"><input id="cm-show-archived" type="checkbox" ${showArchived?'checked':''}>Show archived</label></div>`
  :`<section class="cm-showcase"><div><h1>Build it.<br><span>Make it yours.</span></h1><div class="cm-actions">${b('Create a deck','new-deck',{},true)}${b('Open Deck Lab','open-lab')}${b('Import a list','import-list')}${b('Import a backup','restore')}</div><p class="cm-sub">${howLink}</p></div>${fan}</section>`)+(decks.length?`<div class="cm-deck-grid">${decks.map(d=>{const r=M.readiness(C.state,d);/* THE TILE. The compare tick lives in the top-right corner, always present, so comparing is
...
return `<article class="cm-deck-tile cm-stage-${stage}${picked?' is-picked':''}">${art(d)?`<img class="cm-deck-art" src="${e(art(d))}" alt="" loading="lazy">`:initials(d)}<label class="cm-tile-pick" title="Tick to compare this deck"><input type="checkbox" data-action="compare-pick" data-deck="${e(d.id)}"${picked?' checked':''} aria-label="Compare ${e(d.name)}"></label><button data-action="deck" data-deck="${e(d.id)}"><small>${e(d.name)}</small><h3>${e(commander(d)||'Choose a commander')}</h3>${(()=>{const m=mechanicsOf(d);return m.list.length?`<p${m.derived?' class="cm-tile-derived" title="Read from the list. Name your own in Deck Definition."':''}>${e(m.list.join(' · '))}</p>`:'';})()}<span class="cm-tile-mana">${C.colors(C.state.cards[d.commanders[0]]?.colorIdentity)}</span>${C.pill(e(badge[1]),badge[0])}</button><div class="cm-tile-ready">${C.readinessBar(r)}<span class="cm-tile-caption">${e(caption.join(' · '))}</span></div><footer><span>B${d.definition.baseBracket}${scoreOf(latestReport(d))?` · <span class="cm-badge" title="Latest measured score">${e(scoreOf(latestReport(d)))} pts</span>`:''}</span><span class="cm-muted">${d.locked?'Locked':''}</span><span class="cm-tile-tools"><button type="button" class="cm-icon-button cm-tile-menu-btn" data-action="deck-menu" data-deck="${e(d.id)}" aria-haspopup="menu" aria-label="Deck options for ${e(d.name)}">⋯</button></span></footer></article>`;}).join('')}</div>`:anyDecks?'<p class="cm-muted">Every deck here is archived. Tick Show archived to see them.</p>':'');$('#cm-show-archived')?.addEventListener('change',ev=>{showArchived=ev.target.checked;C.render();});};
...
/* ONE PRIMARY, THREE BESIDE IT, THE REST UNDER MORE. The row carried eight buttons. The three
   that stay are the ones a finalized deck is worked with -- add what you own, buy what you do
   not, log the game -- and a draft's are edit, finalize, log (a deck is often played before
   its reservations exist). Upgrades, a draft's buy list, the card status ladder and
   everything administrative are in More. */
const rowActions=d.archived?b('Restore as draft','restore-deck',{deck:d.id})+b('Delete permanently','delete-deck',{deck:d.id})
  :d.status==='final'?b(pullCount(ready)+ready.remove>0?`Ready to add (${pullCount(ready)+ready.remove})`:'Ready to add','deck-pull',{deck:d.id})+b(`Buy list (${ready.toBuy})`,'deck-buy-list',{deck:d.id})+b('Log a game','log-game',{deck:d.id})
  :b('Edit card list','edit-list',{deck:d.id})+b('Finalize & reserve','finalize',{deck:d.id})+b('Log a game','log-game',{deck:d.id});
C.main.innerHTML=`<section class="cm-deck-hero"${heroArt?` style="--hero:url('${e(heroArt)}')"`:''}><div class="cm-deck-hero-copy"><a class="cm-crumb" href="#decks">My Decks</a><h1>${e(d.name)}</h1><p>${e(commander(d))} ${C.colors(C.state.cards[d.commanders[0]]?.colorIdentity)} <span class="cm-badge ${ready.ready?'good':''}">${d.archived?'Archived':d.status==='draft'?'Defining':ready.complete?'Complete':ready.playable?'Playable':'Building'}</span> <span class="cm-badge">Bracket ${e(String(d.definition.baseBracket))}–${e(String(d.definition.bracketCeiling))}</span>${overCap?` <span class="cm-badge warn" title="Recorded prices of the main list against the definition’s total cap">Over the ${e(C.money(cap))} cap · about ${e(C.money(spend))}</span>`:''}${latest?` <span class="cm-badge" title="Latest measured score">Measured ${e(scoreOf(latest))} pts</span>`:''}${attached(d)?` <span class="cm-badge">Group: ${e(attached(d).name)}</span> <button type="button" class="cm-text-button cm-hero-link" data-action="deck-group" data-group="${e(d.groupId)}">Open group</button>`:''}${d.locked?' <span class="cm-badge warn">Locked</span>':''}</p></div><div class="cm-actions cm-deck-actions">${b('View deck cards','deck-cards',{deck:d.id},true)}${rowActions}${b('More','deck-more-menu',{deck:d.id},false,{caret:'down'})}${C.helpButton('deck')}</div></section>`+stats(d)+/* THREE THINGS YOU DO TO A DECK, AND TWO DRAWERS FOR THE REST. Nine buttons in one row
