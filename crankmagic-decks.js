...
const toGo=r.target-r.owned,badge=d.archived?['draft','Archived']:r.ready?['inbox','Ready to play']:r.playable?['standin',`Playable · ${r.standIns} stand-in${r.standIns===1?'':'s'} · ${toGo} to go`]:['buy',`In progress · ${toGo} to go`];
return `<article class="cm-deck-tile${picked?' is-picked':''}">${art(d)?`<img class="cm-deck-art" src="${e(art(d))}" alt="" loading="lazy">`:initials(d)}<label class="cm-tile-pick" title="Tick to compare this deck"><input type="checkbox" data-action="compare-pick" data-deck="${e(d.id)}"${picked?' checked':''} aria-label="Compare ${e(d.name)}"></label><button data-action="deck" data-deck="${e(d.id)}"><small>${e(d.name)}</small><h3>${e(commander(d)||'Choose a commander')}</h3>${(()=>{const m=mechanicsOf(d);return m.list.length?`<p${m.derived?' class="cm-tile-derived" title="Read from the list. Name your own in Deck Definition."':''}>${e(m.list.join(' · '))}</p>`:'';})()}<span class="cm-tile-mana">${C.colors(C.state.cards[d.commanders[0]]?.colorIdentity)}</span>${C.pill(e(badge[1]),badge[0],'title="Cards not yet owned"')}</button><div class="cm-tile-ready">${C.readinessBar(r)}<span class="cm-tile-caption">${r.sleeved} in box${r.standIns?` (${r.standIns} stand-in${r.standIns===1?'':'s'})`:''} · ${pullCount(r)} pull · ${r.toBuy} buy${(()=>{const g=C.state.games.filter(x=>x.deckId===d.id);if(!g.length)return '';const w=g.filter(x=>x.outcome==='win').length,l=g.filter(x=>x.outcome==='loss').length;return ` · ${w}–${l}`;})()}</span></div><footer><span>B${d.definition.baseBracket}${scoreOf(latestReport(d))?` · <span class="cm-badge" title="Latest measured score">${e(scoreOf(latestReport(d)))} pts</span>`:''}</span><span class="cm-muted">${d.locked?'Locked':''}</span><span class="cm-tile-tools"><button type="button" class="cm-icon-button cm-tile-menu-btn" data-action="deck-menu" data-deck="${e(d.id)}" aria-haspopup="menu" aria-label="Deck options for ${e(d.name)}">⋯</button></span></footer></article>`;}).join('')}</div>`:`<section class="v-panel cm-empty"><h2>A fresh library. A new deck.</h2><p>Start from a commander and let the Lab build the 99, or paste a list you already have. Adding a plan creates no owned copies.</p><div class="cm-actions">${b('Select a commander','open-lab',{},true)}${b('Import a list','import-list')}${b('Import a backup','restore')}</div></section>`);$('#cm-show-archived').addEventListener('change',ev=>{showArchived=ev.target.checked;C.render();});};
