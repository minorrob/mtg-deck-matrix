...
export function publicStack(events){
  ...
  for(const e of events){
    ...
    if(e.kind==='GameEventZone'&&f.zoneType==='Stack'&&f.card){
      ...
      pending.set('stack:'+f.si.stackId,{stackId:f.si.stackId,abilityId:f.sa.abilityId,cardId:c?.cardId,name:c?.faceDown?null:c?.name,faceDown:!!c?.faceDown,playerId:f.si.actor?.playerId??null,kind:f.sa.isSpell?'spell':f.si.isTrigger?'trigger':'ability',stage:'stack',sequence:e.sequence});
