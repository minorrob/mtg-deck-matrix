...
const guestAssets=new Map([
  ...['guest.html','guest.mjs','guest.css','review.html','review.mjs','review.css','setup.mjs','setup.css','online.css','mats.css','playmats.mjs','mana-status.mjs','play-guidance.mjs','action-policy.mjs','card-layout.mjs','handoff.mjs'].map(name=>[name,`game/ui/${name}`]),
...
try{tableRuntime=restoreLocalTableRuntime(runtimeOptions);if(tableRuntime)await tableRuntime.recover();}
  ...
  if(pathname.startsWith('/api/')){
    ...
    try{
      ...
      if(pathname==='/api/ai-session'){
        ...
        aiSession={provider:body.provider,model:body.model,key:stored?.key||body.key,source:stored?.source||'memory-entry'};body.key='';let pilotRearmed=false;const active=liveStatus();if(tableRuntime?.view().phase==='playing')pilotRearmed=!!tableRuntime.armPilots().length;else if(['ready','playing'].includes(active.status))pilotRearmed=startSoloPilots(livePod());return reply(200,{configured:true,provider:aiSession.provider,model:aiSession.model,source:aiSession.source,pilotRearmed});
      }
      if(pathname==='/api/close-game'){stopSoloPilots();return reply(200,await closeLocalGame());}
      ...
      if(pathname==='/api/start'){
        ...
        try{
          if(pod.schema==='CommanderLobbyPack@1'){
            ...
            await tableRuntime.ready(true);hostInvitations=pod.reservedHumanSeats.map(({seatId})=>{const issued=tableRuntime.invite(seatId);return {seatId,expiresIn:issued.expiresIn,link:`${guestInfo.origin}/#table=${encodeURIComponent(issued.tableId)}&invite=${encodeURIComponent(issued.invite)}`};});
