...
export function createApiPilotRunner({seats,bridge,providerForSeat,onEvent=()=>{},pollMs=100}){
  ...
  const state=new Map(seats.map(seat=>[seat.seatId,{seat,policy:pilotPolicy(seat.pilot.difficultyRequested||3),calls:0,lastRevision:-1,errors:0,rejected:new Set(),context:null,pending:null,paused:false,requestController:null}]));
  ...
  return {stop(){stopped=true;abort.abort();},nudge(seatId=null){let prompted=0,waitingForForge=0;for(const entry of state.values()){if(seatId!==null&&entry.seat.seatId!==seatId)continue;prompted++;entry.paused=false;entry.lastRevision=-1;if(entry.requestController){entry.requestController.abort();onEvent({kind:'ai-pilot-prompted',seatId:entry.seat.seatId,action:'cancelled-model-request'});}else if(entry.pending){waitingForForge++;onEvent({kind:'ai-pilot-prompted',seatId:entry.seat.seatId,action:'waiting-for-forge',actionId:entry.pending.request.actionId});}else onEvent({kind:'ai-pilot-prompted',seatId:entry.seat.seatId,action:'recheck-next-decision'});}return {prompted,waitingForForge};},done,status(){return [...state.values()].map(x=>({seatId:x.seat.seatId,difficulty:x.policy.level,providerCalls:x.calls,errors:x.errors,paused:x.paused,pendingActionId:x.pending?.request.actionId,requestInFlight:!!x.requestController}));}};
}
