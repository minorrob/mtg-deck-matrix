...
export function summarizeEvents(events,visibleCards){
  ...
  for(const e of events){
    ...
    else if(kind==='GameEventPlayerDamaged'){source=f.source;actor=f.target?.playerId;label=`${f.amount} ${f.combat?'combat ':''}damage to ${f.target?.name||'a player'}`;}
  ...
  return {counts,classified,proliferateInstrumented,cards:[...cards.values()],recent:recent.reverse(),scope:'Observed public events and your revealed cards; raw private journal data is excluded.'};
}
const cache=new Map();
export function matchTelemetry(directory,state){
  ...
  for(const event of entry.events){if(event.sequence<=entry.last)continue;entry.last=event.sequence;
    ...
    if(['manifest','mechanic-choice-completed','GameEventTurnPhase','GameEventPlayerLivesChanged','GameEventShuffle','GameEventPlayerPoisoned','GameEventManaPool','GameEventAttackersDeclared','GameEventBlockersDeclared'].includes(event.kind))entry.accepted.push(event);
