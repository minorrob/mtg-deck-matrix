...
createServer(async(req,res)=>{
  ...
  try{
    ...
    const key=JSON.parse(body).key;body='';const provider=createOpenAIChoiceProvider({apiKey:key});
    running=true;status='Running isolated Forge and live OpenAI integration check…';reply(202,{status});
    runEngineProof({provider}).then(r=>{status=r.passed?'PASS: live OpenAI choice returned through the per-seat Forge bridge. Test engine closed.':'Proof did not pass';})
