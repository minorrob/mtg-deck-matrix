...
const deckDraft={source:'upload',name:'',csv:'',deckId:'',commander:'',archidektCommander:'',url:'',handoff:null};
let deckWindow=null,deckRequestId=null,deckRequestTimer=null;
...
function deckPanel(table,seat){
  ...
  const source=el('select');source.setAttribute('aria-label','Deck source');for(const [value,label]of [['crankmagic','Select one of your CrankMagic decks'],['upload','Upload Moxfield two-column CSV'],['preloaded','Use a preloaded CrankMagic deck'],['lab','Build from a commander with Deck Lab'],['archidekt','Load an Archidekt deck']]){const option=el('option','',label);option.value=value;source.append(option);}
  ...
  const crankmagic=el('div','choice'),handoffStatus=el('p','fine',deckDraft.handoff?`Selected: ${deckDraft.handoff.name} · ${deckDraft.handoff.commanders.join(' + ')}`:'Open your own CrankMagic library, choose a deck, then return here automatically.');
  const requestDeck=button(deckDraft.handoff?'Choose a different CrankMagic deck':'Open my CrankMagic decks',()=>{
    deckRequestId=crypto.randomUUID();const libraryOrigin='https://minorrob.github.io',target=libraryOrigin+'/mtg-deck-matrix/#decks';
    deckWindow=window.open(target,'crankmagic-deck-library');if(!deckWindow){message('Allow pop-ups to choose a deck from CrankMagic.',true);return;}
    const send=()=>{try{deckWindow?.postMessage({type:'crankmagic-online-deck-request',requestId:deckRequestId},libraryOrigin);}catch{}};send();clearInterval(deckRequestTimer);let attempts=0;deckRequestTimer=setInterval(()=>{send();if(++attempts>=15)clearInterval(deckRequestTimer);},500);
    handoffStatus.textContent='Choose a deck in the CrankMagic tab, then select Use in invited game.';
  },'primary');
  crankmagic.append(requestDeck,handoffStatus);
  ...
  const choices={crankmagic,upload,catalog,build,archidekt},showSource=()=>{for(const n of Object.values(choices))n.classList.remove('active');choices[source.value==='preloaded'?'catalog':source.value==='lab'?'build':source.value].classList.add('active');};source.addEventListener('change',()=>{deckDraft.source=source.value;showSource();});showSource();
  const save=button('Validate and use this deck',async()=>{try{save.disabled=true;message('Resolving all 100 cards, mechanics, bracket and table budget…');let input={source:deckDraft.source,name:deckDraft.name};if(deckDraft.source==='crankmagic'){if(!deckDraft.handoff)throw Error('Open your CrankMagic library and choose a deck first.');input.name=deckDraft.handoff.name;input.handoff=deckDraft.handoff;}else if(deckDraft.source==='upload')input.csv=deckDraft.csv;else if(deckDraft.source==='preloaded'){input.deckId=saved.value;input.commander=saved.selectedOptions[0]?.dataset.commander;deckDraft.deckId=saved.value;}else if(deckDraft.source==='lab')input.commander=deckDraft.commander;else{input.commander=deckDraft.archidektCommander;input.archidektUrl=deckDraft.url;}await api('/table/deck',{method:'POST',body:input});await refresh();message('Deck validated. Press Ready when you are finished choosing.');}catch(error){message(error.message,true);}finally{save.disabled=false;}},'primary');
  panel.append(el('div','deck-fields'),source,name,crankmagic,upload,catalog,build,archidekt,save);return panel;
}
window.addEventListener('message',event=>{
  if(event.origin!=='https://minorrob.github.io'||event.source!==deckWindow)return;
  const value=event.data;if(value?.type!=='crankmagic-online-deck-selected'||value.requestId!==deckRequestId||value.handoff?.schema!=='CrankMagicDeckHandoff@1')return;
  deckDraft.handoff=value.handoff;deckDraft.source='crankmagic';clearInterval(deckRequestTimer);deckWindow?.close();deckWindow=null;message('Selected '+value.handoff.name+'. Validate it to seat this deck.');refresh();
});
