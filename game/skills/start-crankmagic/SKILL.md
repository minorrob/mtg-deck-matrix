...
1. Run `game/tools/start-crankmagic.ps1` from that checkout. It keeps a healthy existing host and starts a hidden persistent process otherwise. On Windows it makes the named `crankmagic_openai_api` Credential Manager entry available only to the local host process; it never prints the value. Do not replace a running game or kill a process just to free the port.
...
The prepared engine and JDK paths are documented in `game/README.md`. This workflow starts the local host; selecting an API pilot may make paid OpenAI requests under the match budget. Never print session tokens, private hands, credential values, or AI keys. Keep diagnostics in ignored `game/.local/host/`. If an older host occupies the port, inspect its live status and explain the need to finish or explicitly end that game before restarting it.
