...
test('stored OpenAI configuration defaults to GPT-5 Mini with GPT-5 as the only promoted fallback',()=>{
  const session=loadOpenAiCredential('crankmagic_openai_api',{platform:'win32',run:()=>'test-key-that-is-long-enough'});
  assert.equal(session.provider,'openai');assert.equal(session.model,'gpt-5-mini');assert.equal(session.source,'windows-credential-manager');
  assert.equal(DEFAULT_OPENAI_MODEL,'gpt-5-mini');assert.equal(OPENAI_FALLBACK_MODEL,'gpt-5');assert.deepEqual(OPENAI_MODELS,['gpt-5-mini','gpt-5']);
