...
test('invite redemption is durable, single-use and authorizes only the claimed private seat',async()=>{
  ...
  const member=await broker.authenticate(joined.capability);await broker.deck(member,{deckVersion:deck('friend-v1')});await broker.ready(member,{ready:true});await broker.beginCountdown();now+=10000;await broker.tick();await broker.processOutbox();
...
test('deck readiness, countdown and launch outbox survive reload without duplicate engines',async()=>{
  ...
  await broker.deck(member,{deckVersion:deck('friend-v1')});await broker.ready(member,{ready:true});await broker.beginCountdown();now+=10000;await broker.tick();
...
test('a launch marked running before a broker restart resumes the same persisted match command',async()=>{
  ...
  let broker=new TableBroker({file,table,clock:()=>now,bridge:async()=>({}),launch:async({matchId})=>matchId});await broker.beginCountdown();now=10000;await broker.tick();const saved=JSON.parse(readFileSync(file));saved.outbox[0].status='running';writeFileSync(file,JSON.stringify(saved));const expected=saved.outbox[0].matchId;
...
test('leaving an active game concedes only the authenticated engine seat and revokes it',async()=>{
  ...
  const invite=broker.invite(1),session=await broker.join(invite),member=await broker.authenticate(session.capability);await broker.deck(member,{});await broker.ready(member,{ready:true});await broker.beginCountdown();now=10000;await broker.tick();await broker.processOutbox();
...
test('completed feedback is durable, scoped to the authenticated seat and tied to the match deck version',async()=>{
  ...
  let broker=new TableBroker({file,table,clock:()=>now,bridge:async seat=>({viewerSeatId:seat,viewerPlayerId:seat}),launch:async({matchId})=>matchId,report});await broker.beginCountdown();now=10000;await broker.tick();await broker.processOutbox();const matchId=broker.hostView().matchId;await broker.complete(matchId);
