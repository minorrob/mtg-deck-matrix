...
test('mixed lobby joins, validates its own deck, counts down once and routes its private view',async t=>{
  ...
  assert.equal(runtime.view().phase,'countdown');assert.equal(runtime.view().countdownAt,5100);
...
test('unanimous rematch keeps capabilities and prior deck versions while clearing readiness',async t=>{
  ...
  await runtime.guest.deck(member,{});await runtime.ready(true);await runtime.guest.ready(member,{ready:true});now=5000;await runtime.poll();await runtime.broker.complete(matchId);
