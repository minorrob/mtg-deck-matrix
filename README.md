...
- [CrankMagic: the game](docs/crankmagic-game-plan.md) — a real four-player Commander game on the play-space table: your deck against 1–3 opponents piloted by Claude, decks from your library / an Archidekt link / a paste / the simulator, one bracket and Game Changer cap enforced on every seat. A referee that never guesses, a three-tier card ladder so an arbitrary decklist is playable, playbooks generated from the graph's loops and chains, and ~35 model calls a game rather than 400. **G0, the lobby, is built** (`crankmagic-lobby.js` and the Play page): your deck plus one to three opponents from the library, an Archidekt link, a paste or a list generated to the table's bracket and a budget, one bracket judging every seat with Trim and Change the bracket as the two ways forward, and the pod's balance read against the rest of the field. G1 onward waits on five decisions. [`docs/mockups/game-plan-review.html`](docs/mockups/game-plan-review.html) is the same plan as a standalone page you open from disk — a live lobby, a live cost calculator, and the five open decisions as questions you can answer and copy back
