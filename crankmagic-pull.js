...
views.pull=async params=>{const d=M.deck(C.state,params.get('deck')||'');if(done.deckId!==d.id)done={deckId:d.id,rows:new Map()};
  ...
  C.main.innerHTML=`<div class="cm-pull"><header class="cm-page-head cm-pull-head"><div><div class="v-eyebrow cm-eyebrow-warm">Ready to add</div><h1>${e(d.name)}</h1><p class="cm-pull-counts"><span>${r.sleeved} in the physical deck${r.standIns?` (${r.inBox} of the list)`:''}</span> · <span>${live(groups.bench).reduce((n,x)=>n+x.quantity,0)} from bench</span> · <span>${live(groups.other).reduce((n,x)=>n+x.quantity,0)} from other decks</span> · <span>${r.standIns} substitute${r.standIns===1?'':'s'}${r.remove?` (${r.remove} to take out now)`:''}</span> · <span>${waiting.ordered} ordered</span> · <span>${waiting.toBuy} to buy</span></p></div><div class="cm-actions">${b('Open deck','deck',{deck:d.id})}${b('Print','pull-print')}${b('Export','pull-export',{deck:d.id})}${left?b('Mark all added','pull-all',{deck:d.id},true):''}</div></header>`
