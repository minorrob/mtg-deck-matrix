// Builds a simulation request from a baked variant's Tuned or Maxed build, so a
// deck can be simulated without downloading anything from the browser.
//
...
import {parseArgs, readJson, writeJson, loadCatalog, loadConfig, loadOpponents, tunedCards, maxedCards, validateList, requestIdFor, stampNow, relative, ROOT, SIM_DIR} from "./lib.mjs";

const args = parseArgs(process.argv.slice(2));

if (args.help || (!args.variant && !args.list)) {
  console.log("Usage: node tools/sim/make-request.mjs --variant <variantId> [--stage tuned|maxed] [--table <name>] [--out <file>]");
...
// Tuned is checked against Tier 2 (no Game Changers, no mass land denial, no
// two-card combos); Maxed intentionally pushes to the Tier 3 line (up to three
// Game Changers) — that pairing matches how the site already frames the two
// stages, so the simulator enforces the rules each stage is actually meant to
// satisfy rather than checking every build against one tier by default.
const stageArg = String(args.stage || "tuned").toLowerCase();
if (!["tuned", "maxed"].includes(stageArg)) throw new Error(`--stage must be "tuned" or "maxed", got "${stageArg}"`);
const stage = stageArg === "maxed" ? "Maxed" : "Tuned";
const tier = stageArg === "maxed" ? 3 : 2;
const cards = stageArg === "maxed" ? maxedCards(plan, audited) : tunedCards(plan, audited);
