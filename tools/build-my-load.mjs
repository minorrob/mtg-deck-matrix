...
//
//   node tools/build-my-load.mjs                       # derive it from data/archive/active-state.json
...
//
// WITHOUT --from it is derived from data/archive/active-state.json, which is the repository's own
...
const OUT = "data/archive/my-load.json";
...
if (from) {
  ...
  const active = JSON.parse(await readFile("data/archive/active-state.json", "utf8"));
  if (!active.state || typeof active.state !== "object") {
    throw new Error("data/archive/active-state.json has no state block to build a default from.");
  }
  ...
  const file = {
    ...
    note: "Derived from data/archive/active-state.json. Replace it with a fresh export from the app; "
      + "see tools/build-my-load.mjs.",
    source: {from: "data/archive/active-state.json", exportedAt: active.exportedAt || null},
