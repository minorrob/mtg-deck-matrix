...
(function(root,factory){const api=factory();if(typeof module==='object'&&module.exports)module.exports=api;if(root)root.CrankCollection=api;})(typeof globalThis!=='undefined'?globalThis:this,function(){
  ...
  function projection(s){const rows=s.lots.map(l=>withDeckGroup(s,{...clone(l),recordId:l.id,kind:'lot',card:card(s,l.cardId),deckId:l.allocation?.deckId||'',purpose:l.allocation?slot(s,l.allocation.deckId,l.allocation.slotId).purpose:'',placement:inDeck(s,l)?'In deck':l.allocation?'Reserved':l.source==='owned'?'Bench':'Unassigned',physical:physical(l)}));for(const d of s.decks.filter(d=>d.status==='final'&&!d.archived))for(const r of d.slots.filter(r=>r.committed)){const need=shortfall(s,d,r);if(need)rows.push(withDeckGroup(s,{recordId:`need:${d.id}:${r.id}`,kind:'need',deckId:d.id,slotId:r.id,cardId:r.cardId,card:card(s,r.cardId),source:'to-buy',quantity:need,purpose:r.purpose,printing:clone(r.printing||{}),placement:'Reserved',physical:'Not acquired',offer:'none',groupIds:[]}));}return rows;}
