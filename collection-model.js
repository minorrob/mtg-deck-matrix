...
(function(root,factory){const api=factory();if(typeof module==='object'&&module.exports)module.exports=api;if(root)root.CrankCollection=api;})(typeof globalThis!=='undefined'?globalThis:this,function(){
  ...
  /* 'wanted' is a card you mean to buy with no order placed and no vendor chosen -- the note
     you make at a convention table. It is a plan, not a copy: never physical, never
     eligible for a build, never reserved to a slot; it becomes ordered or owned by the
     same 'source' correction the other kinds use. */
  const VERSION=1, SOURCES=['owned','ordered','incoming','wanted'], PURPOSES=['main','upgrade','bracket'];
  ...
  function physical(l){return l.source==='owned'?(l.location?.kind==='deck'?'In deck':'Bench'):l.source==='wanted'?'Not acquired':'Not received';}
  ...
  function counters(s){const result={owned:0,ordered:0,incoming:0,wanted:0,toBuy:0,inDeck:0,sellTrade:0};for(const l of s.lots){result[l.source]+=l.quantity;if(inDeck(s,l))result.inDeck+=l.quantity;if(l.source==='owned'&&l.offer!=='none')result.sellTrade+=l.quantity;}for(const d of s.decks.filter(d=>d.status==='final'&&!d.archived))for(const r of d.slots.filter(r=>r.committed))result.toBuy+=shortfall(s,d,r);return result;}
  ...
  function newLot(raw){card(s,raw.cardId);const l={id:raw.id||id('lot'),cardId:raw.cardId,quantity:quantity(raw.quantity),source:raw.source||'owned',printing:print(raw.printing),location:raw.source&&raw.source!=='owned'?null:clone(raw.location||{kind:'bench',box:''}),allocation:null,offer:'none',groupIds:[],notes:text(raw.notes,5000),paid:raw.paid??null,acquiredAt:now,provenance:clone(raw.provenance||{type:c.type,operation:c.id})};s.lots.push(l);return l;}
    ...
    function satisfy(d,only){for(const r of d.slots.filter(r=>r.committed&&(!only||r.id===only))){for(const l of [...s.lots].sort((a,b)=>(a.source==='owned'?0:1)-(b.source==='owned'?0:1)||filedFor(a,d)-filedFor(b,d))){if(!shortfall(s,d,r))break;if(l.source==='wanted'||l.allocation||l.offer==='held'||l.location?.kind==='deck'||l.offer==='available'&&d.definition.reuse?.includeSellTrade===false||!compatible(l,r)||l.keepBench)continue;allocate(l,d,r,Math.min(l.quantity,shortfall(s,d,r)));}}}
    ...
    switch(c.type){
      ...
      case 'createDeck':{for(const raw of c.cards||[])addCard(raw);const d={id:c.deckId||id('deck'),name:text(c.name,160)||'Untitled deck',commanders:[...(c.commanders||[])],slots:rows(c.slots||[]),definition:defaultDefinition(c.definition),status:'draft',archived:false,locked:false,version:1,versions:[],createdAt:now,notes:text(c.notes,5000),priority:s.decks.length,groupId:c.groupId?group(s,c.groupId).id:null};s.decks.push(d);summary=`Created planned deck: ${d.name}`;break;}
      case 'editDeck':{const d=deck(s,c.deckId);ensure(!d.archived,'Restore the archived deck first.');if(c.name!==undefined)d.name=text(c.name,160);if(c.definition)d.definition=defaultDefinition(c.definition);if(c.notes!==undefined)d.notes=text(c.notes,5000);if(c.groupId!==undefined)d.groupId=c.groupId?group(s,c.groupId).id:null;if(c.commanders){ensure(d.status==='draft','Use a reviewed commander replacement for a finalized list.');d.commanders=[...c.commanders];}if(c.slots){ensure(d.status==='draft','Use a reviewed replacement for a finalized list.');d.slots=rows(c.slots);}summary=`Updated ${d.name}`;break;}
      ...
      /* THE LAST STEP OF THE BUY. To buy -> ordered -> owned -> reserved was a flow you
         could finish, and the hundredth card still was not in the deck, because the physical
         move was one confirmation per copy and nobody confirms a hundred of them. A deck goes
