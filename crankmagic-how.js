...
const STEPS=[
  ...
  {n:5,title:'Assemble',href:'#collection',text:'Pull the cards into the box. Stand-ins hold seats until the real ones arrive.'},
  {n:6,title:'Play',href:'#decks',text:'All 100 in the box: the deck is done. Log your games.'}];
const LADDER=[['To buy','The list wants it. You have no copy yet.'],['Watching','Keeping an eye on it.'],['Wanted','On your shopping list.'],['Ordered','Paid for, or on its way in a trade.'],['Owned','In hand, on the bench.'],['In the box','Sleeved in the deck.']];
views.how=async()=>{
  C.main.innerHTML=C.head('Six steps','How a deck comes together.','Start either way. Every step opens where it begins.',`<a class="v-button" href="#decks">Back to My Decks</a>`)
   +`<ol class="cm-how-flow" aria-label="The six steps">
    ...
    <section class="cm-how-ladder" aria-label="What a card's status means">
      ...
      <p class="cm-muted cm-how-note">A <strong>stand-in</strong> is a different card holding a seat in the box until the real one arrives.</p>
    </section>`;
};
});
