...
const PILL_KIND={'Physical deck':'inbox',Substitute:'standin',Reserved:'reserved',Bench:'pull',Unassigned:'draft','Draft list':'draft',Suggestion:'draft',Planned:'draft',Owned:'pull',owned:'pull',Ordered:'ordered',ordered:'ordered','to-buy':'buy','To buy':'buy',Watched:'watch',watching:'watch',draft:'draft',remove:'remove'};
...
async function render(){if(!catalog)return;describeData();disposeView?.();disposeView=null;glossary?.hide();const seq=++renderSeq,r=route();const group=NAV_GROUP[r.view]||r.view;document.querySelectorAll('[data-nav]').forEach(el=>el.setAttribute('aria-current',el.dataset.nav===group?'page':'false'));try{subnav(group);}catch(e){console.error(e);}try{const cleanup=await views[r.view](r.params);if(seq===renderSeq)disposeView=cleanup||null;else cleanup?.();}catch(err){if(seq===renderSeq)main.innerHTML=head('Unable to open this view','Your saved library is intact',err.message,button('Decks','home'));}status();}
function status(){if(repo)$('#cm-save-status').textContent=`Saved locally · revision ${state.revision}${navigator.onLine?'':' · offline'}`;}
async function refresh(){state=await repo.getState();for(const c of Object.values(state.cards))catalog?.overlay(c);await render();}
async function commit(command,{renderView=true}={}){if(committing)throw Error('A save is already in progress. Please wait for its receipt.');committing=true;$('#cm-save-status').textContent='Saving…';try{const result=await repo.commit({id:uid(),...command},state.revision);state=result.state;for(const c of Object.values(state.cards))catalog.overlay(c);notice(result.summary);if(renderView)await render();else status();await writeMirror();return result;}catch(error){state=await repo.getState();status();throw error;}finally{committing=false;}}
...
const glossaryView={html:text=>glossary&&termsOn()?glossary.html(text):esc(text),hide(){glossary?.hide();},get entries(){return glossary?.entries||[];}};
...
actions['page-help']=el=>{const h=HELP[el.dataset.help];if(h)modal(h.title,h.body);};
...
actions.close=()=>dialog.close();actions.home=()=>go('decks');actions.card=el=>inspector(el.dataset.card);actions['library-card']=el=>{dialog.close();go('cards',{card:el.dataset.card});};actions['discover-card']=el=>{dialog.close();go('discover',{card:el.dataset.card});};actions['reset-picks']=()=>commit({type:'preferences',values:{comparisonPicks:[]}});
...
/* SHARE. Three ways to hand the app to someone, none of them needing a server: a subscription
...
window.addEventListener('hashchange',()=>{const key=routeKey();if(key!==lastKey)scrollTo(0,0);lastKey=key;render();main.focus({preventScroll:true});});window.addEventListener('online',status);window.addEventListener('offline',status);
