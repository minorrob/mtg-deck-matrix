...
function deckLine(deckId,next){
  ...
  return `<p>In ${esc(d.name)}: <strong>${move(a.target,b.target)}</strong> planned · <strong>${move(a.owned,b.owned)}</strong> owned &amp; reserved · <strong>${move(a.placed,b.placed)}</strong> in the deck box · <strong>${move(a.toBuy,b.toBuy)}</strong> to buy</p>`;
}
