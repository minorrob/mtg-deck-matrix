...
const PILL_KIND={'In physical deck':'inbox',Substitute:'standin',Reserved:'pull',Bench:'pull',Unassigned:'draft','Draft list':'draft',Suggestion:'draft',Planned:'draft',Owned:'pull',owned:'pull',Ordered:'ordered',ordered:'ordered','to-buy':'buy','To buy':'buy',Watched:'watch',watching:'watch',draft:'draft',remove:'remove'};
function pillKind(key,placement){if((key==='owned'||key==='Owned')&&placement==='In physical deck')return 'inbox';return PILL_KIND[key]||'draft';}
...
function readinessBar(r){const inBox=r.inBox??r.placed??0,pull=(r.pullFromBench||0)+(r.pullFromOtherBox||0),way=r.ordered||0,seg=[['inbox',inBox,'in box'],['pull',pull,'to pull'],['ordered',way,'on the way'],['buy',r.toBuy||0,'to buy']];
  const whole=seg.reduce((n,[,k])=>n+k,0)||r.target||1,standIn=Math.min(r.standIns||0,Math.max(0,whole-inBox));
  return `<div class="cm-readiness" role="img" aria-label="${esc(seg.map(([,n,l])=>`${n} ${l}`).join(', ')+(standIn?`, ${standIn} substitute${standIn===1?'':'s'} filling seats`:'')+` of ${r.target}`)}">${seg.map(([k,n])=>n>0?`<i class="${k}" style="flex:${n} 1 0"></i>`:'').join('')}${standIn?`<b class="standin" style="left:${inBox/whole*100}%;width:${standIn/whole*100}%"></b>`:''}</div>`;}
...
function readinessLegend(r){const pull=(r.pullFromBench||0)+(r.pullFromOtherBox||0),way=r.ordered||0;return `<p class="cm-readiness-legend"><span><i class="inbox"></i>${r.inBox??r.placed??0} ${r.standIns?'of the list ':''}in the physical deck</span><span><i class="pull"></i>${pull} to pull</span><span><i class="ordered"></i>${way} on the way</span><span><i class="buy"></i>${r.toBuy||0} to buy</span>${r.standIns?`<span><i class="standin"></i>${r.standIns} substitute${r.standIns===1?'':'s'}</span>`:''}</p>`;}
...
/* THE CARD'S STANDING IN YOUR LIBRARY, IN TWO LINES. Status is where its copies sit on the
   ladder -- how many Owned, Ordered, Watched -- plus what finalized decks
   still want (To buy). Assignment is where those copies are: in a physical deck, reserved to a
...
function standing(id){
  ...
  const status=[['Owned','owned'],['Ordered','ordered'],['Watched','watching']].map(([l,k])=>[l,by(r=>r.kind==='lot'&&r.source===k)]).concat([['To buy',by(r=>r.kind==='need')]]);
  const assignment=[['In physical deck',by(r=>r.kind==='lot'&&r.placement==='In physical deck')],['Substitute',by(r=>r.kind==='lot'&&r.placement==='Substitute')],['Reserved',by(r=>r.kind==='lot'&&r.placement==='Reserved')],['Bench',by(r=>r.kind==='lot'&&r.placement==='Bench')],['Unassigned',by(r=>r.kind==='lot'&&r.placement==='Unassigned')],['Draft list',draft.reduce((n,x)=>n+x.quantity,0)],['Planned',planned.reduce((n,x)=>n+x.quantity,0)],['Option',by(r=>r.kind==='lot'&&r.option)],['Pinned',by(r=>r.kind==='lot'&&r.pinned)]];
  ...
  const lines=[...rows.map(r=>`<p>${line([source(r.source),String(r.quantity),r.placement,r.deckId?M.deck(state,r.deckId).name:'',r.option?'Option — first to swap out':'',r.pinned?'Pinned':'',r.kind==='lot'?readableLocation(r):''])}</p>`),...draft.map(x=>`<p>Draft list · ${x.quantity} · ${esc(x.deck.name)}</p>`),...planned.map(x=>`<p>Planned · ${x.quantity} · ${esc(x.group.name)}</p>`)];
