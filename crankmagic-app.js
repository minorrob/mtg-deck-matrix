...
function readinessLegend(r){const pull=(r.pullFromBench||0)+(r.pullFromOtherBox||0),way=(r.ordered||0)+(r.incoming||0);return `<p class="cm-readiness-legend"><span><i class="inbox"></i>${r.inBox??r.placed??0} in box</span><span><i class="pull"></i>${pull} to pull</span><span><i class="ordered"></i>${way} on the way</span><span><i class="buy"></i>${r.toBuy||0} to buy</span>${r.standIns?`<span><i class="standin"></i>${r.standIns} stand-in${r.standIns===1?'':'s'}</span>`:''}</p>`;}
