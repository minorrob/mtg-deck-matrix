...
function standing(id){
  ...
  const chips=list=>list.filter(([,n])=>n>0).map(([l,n])=>pill(`${esc(l)} <strong>${n}</strong>`,pillKind(l))).join('')||'<span class="cm-muted">none</span>';
