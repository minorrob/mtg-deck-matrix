...
function standing(id){
  ...
  const chips=list=>list.filter(([,n])=>n>0).map(([l,n])=>`<span class="cm-chip">${esc(l)} <strong>${n}</strong></span>`).join('')||'<span class="cm-muted">none</span>';
  const lines=[...rows.map(r=>`<p>${esc(source(r.source))} · ${r.quantity} · ${esc(r.placement)}${r.deckId?' · '+esc(M.deck(state,r.deckId).name):''}${r.kind==='lot'?' · '+esc(readableLocation(r)):''}</p>`),...draft.map(x=>`<p>Draft list · ${x.quantity} · ${esc(x.deck.name)}</p>`),...planned.map(x=>`<p>Planned · ${x.quantity} · ${esc(x.group.name)}</p>`)];
