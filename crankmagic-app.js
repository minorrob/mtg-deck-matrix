...
const PILL_KIND={'In deck box':'inbox','Stand-in':'standin',Reserved:'pull',Bench:'pull',Unassigned:'draft','Draft list':'draft',Suggestion:'draft',Planned:'draft',Owned:'pull',owned:'pull',Ordered:'ordered',ordered:'ordered','Incoming trade':'ordered',incoming:'ordered',Wanted:'buy',wanted:'buy','to-buy':'buy','To buy':'buy',Watching:'watch',watching:'watch',draft:'draft',remove:'remove'};
...
function readinessBar(r){const inBox=r.inBox??r.placed??0,pull=(r.pullFromBench||0)+(r.pullFromOtherBox||0),way=(r.ordered||0)+(r.incoming||0),seg=[['inbox',inBox,'in box'],['pull',pull,'to pull'],['ordered',way,'on the way'],['buy',r.toBuy||0,'to buy']];
  const whole=seg.reduce((n,[,k])=>n+k,0)||r.target||1,standIn=Math.min(r.standIns||0,Math.max(0,whole-inBox));
  return `<div class="cm-readiness" role="img" aria-label="${esc(seg.map(([,n,l])=>`${n} ${l}`).join(', ')+(standIn?`, ${standIn} stand-in${standIn===1?'':'s'} filling seats`:'')+` of ${r.target}`)}">${seg.map(([k,n])=>n>0?`<i class="${k}" style="flex:${n} 1 0"></i>`:'').join('')}${standIn?`<b class="standin" style="left:${inBox/whole*100}%;width:${standIn/whole*100}%"></b>`:''}</div>`;}
...
function readinessLegend(r){const pull=(r.pullFromBench||0)+(r.pullFromOtherBox||0),way=(r.ordered||0)+(r.incoming||0);return `<p class="cm-readiness-legend"><span><i class="inbox"></i>${r.inBox??r.placed??0} ${r.standIns?'of the list ':''}in box</span><span><i class="pull"></i>${pull} to pull</span><span><i class="ordered"></i>${way} on the way</span><span><i class="buy"></i>${r.toBuy||0} to buy</span>${r.standIns?`<span><i class="standin"></i>${r.standIns} stand-in${r.standIns===1?'':'s'}</span>`:''}</p>`;}
