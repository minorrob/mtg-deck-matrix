...
const PILL_KIND={'In physical deck':'inbox',Substitute:'standin',Reserved:'reserved',Bench:'pull',Unassigned:'draft','Draft list':'draft',Suggestion:'draft',Planned:'draft',Owned:'pull',owned:'pull',Ordered:'ordered',ordered:'ordered','to-buy':'buy','To buy':'buy',Watched:'watch',watching:'watch',draft:'draft',remove:'remove'};
...
function readinessLegend(r){const pull=(r.pullFromBench||0)+(r.pullFromOtherBox||0),way=r.ordered||0;return `<p class="cm-readiness-legend"><span><i class="inbox"></i>${r.inBox??r.placed??0} ${r.standIns?'of the list ':''}in the physical deck</span><span><i class="pull"></i>${pull} ready to add</span><span><i class="ordered"></i>${way} on the way</span><span><i class="buy"></i>${r.toBuy||0} to buy</span>${r.standIns?`<span><i class="standin"></i>${r.standIns} substitute${r.standIns===1?'':'s'}</span>`:''}</p>`;}
...
function standing(id){
  ...
  for(const r of rows){if(r.kind==='need')add(r.deckId,'To buy','buy',r.quantity);else if(r.kind==='lot'){if(r.placement==='In physical deck')add(r.deckId,'In physical deck','inbox',r.quantity);else if(r.placement==='Substitute')add(r.standInDeckId,'Substitute','standin',r.quantity);else if(r.placement==='Reserved')add(r.deckId,r.source==='ordered'?'Ordered':'Reserved',r.source==='ordered'?'ordered':'reserved',r.quantity);}}
  for(const x of draft)add(x.deck.id,'Draft list','draft',x.quantity);
  const open=`data-action="library-card" data-card="${esc(id)}"`;
  const chips=list=>list.filter(([,n])=>n>0).map(([l,n])=>`<button type="button" class="cm-pill ${esc(pillKind(l))}" ${open} title="Open the Collection filtered to this card">${esc(l)} <strong>${n}</strong></button>`).join('')||'<span class="cm-muted">none</span>';
  const assignment=[...decks.values()].map(x=>`<button type="button" class="cm-pill ${esc(x.kind)}" ${open} title="${esc(x.label)} · ${x.quantity} — open the Collection filtered to this card">${esc(x.name)}${x.quantity>1?` <strong>${x.quantity}</strong>`:''}</button>`).concat(planned.map(x=>`<button type="button" class="cm-pill draft" ${open} title="Planned in the group ${esc(x.group.name)}">${esc(x.group.name)} · planned${x.quantity>1?` <strong>${x.quantity}</strong>`:''}</button>`)).join('');
