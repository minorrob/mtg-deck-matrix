...
function batchBar(){
  const n=picked.size;
  if(!n)return '';
  return `<div class="cm-batch-bar"><strong>${n} record${n===1?'':'s'} ticked</strong>${b('Set status','batch-status',{},true,{caret:'down'})}${b('Ordered…','batch-order')}${b('Bought in store','batch-store')}${b('Arrived','batch-arrived')}${b('Add to a group','batch-group')}${b('Put in a deck box','batch-place')}${b('Move physically to Bench','batch-bench')}${b('Release reservation → To buy','batch-release')}${b('Flag ▾','batch-flag')}${b('Offer for Sell / Trade','batch-offer')}<button type="button" class="cm-text-button" data-action="batch-clear">Clear</button></div>`;
}
...
const SHEET_SHOW=[['','All cards'],['listed','In a deck list'],['short','Short of a target'],['pending','Reserved, not in the box yet'],['subs','Stand-ins in a box'],['owned','Owned copies'],['bench','On the bench'],['loose','In no deck list']];
...
actions['sheet-csv']=()=>{const m=M.matrix(C.state),rows=sheetRows(m),decks=m.decks,cell=v=>/[",\r\n]/.test(String(v))?'"'+String(v).replace(/"/g,'""')+'"':String(v);
  ...
  C.download(`CrankMagic-spreadsheet-${new Date().toISOString().slice(0,10)}.csv`,lines.join('\r\n'),'text/csv;charset=utf-8');C.notice(`Exported ${rows.length} row${rows.length===1?'':'s'}: Card, Own, Buy Count, Ordered, Bench, then T and A per deck. A is what is physically in the box, stand-ins included, like the Master's Actual.`);};
...
const cell=(r,k)=>{
  ...
  const body=mixed?'<span class="cm-muted">Various</span>'
    ...
    :e(value(r,k)??'Unknown');
  const standInBadge=k==='placement'&&r.standIn&&r.placement!=='Stand-in'?` <span class="cm-badge cm-badge-standin" title="Physically in ${e(M.deck(C.state,r.standInDeckId).name)}'s box, standing in there until it is pulled">Standing in · ${e(M.deck(C.state,r.standInDeckId).name)}</span>`:'';
...
actions['batch-place']=()=>{
  ...
  form('Put these copies in a deck box',s('Deck','deckId',decks.map(d=>[d.id,d.name]),'')+f('Box label (optional)','box')+`<label class="cm-checkbox cm-full"><input type="checkbox" name="asStandIn"> Allow stand-ins: a copy this deck's list does not call for goes in unreserved, filling a seat until the real card arrives</label>`+note('Records where these copies physically are. Ownership does not change. A ticked copy that is not reserved for this deck is refused by name unless stand-ins are allowed; one the list calls for is reserved on the way in.'),
    v=>C.review('Put these copies in a deck box',note(`${lotIds.length} record${lotIds.length===1?'':'s'} move into ${e(M.deck(C.state,v.deckId).name)}${v.asStandIn?', as stand-ins where the list does not call for them':''}.`),{type:'bulk',op:'place',deckId:v.deckId,box:v.box,lotIds,...(v.asStandIn?{asStandIn:true}:{})}),'Review placement');
};
...
function primary(r){if(r.kind==='fold'){const parts=r.partRows;if(parts.every(p=>p.kind==='need'||p.kind==='lot'&&M.PLANNED.includes(p.source)))return `<button class="v-button primary compact cm-row-buy" data-action="shop-buy" data-record="${e(r.recordId)}">Bought</button><button class="v-button compact cm-row-buy" data-action="shop-order" data-record="${e(r.recordId)}">Ordered</button>`;if(parts.every(p=>p.kind==='lot'&&(p.source==='ordered'||p.source==='incoming')))return `<button class="v-button primary compact cm-row-buy" data-action="shop-arrive" data-record="${e(r.recordId)}">Arrived</button>`;return '';}
  ...
  if(r.kind==='lot'&&r.source==='owned'&&r.allocation&&r.placement!=='In deck box'){const d=M.deck(C.state,r.allocation.deckId);return `<button class="v-button primary compact cm-row-buy" data-action="place-now" data-record="${e(r.recordId)}" title="Record that this copy is in ${e(d.name)}’s box">Put in ${e(shortDeck(d))} box</button>`;}
...
actions['place-now']=async el=>{const r=findRow(el.dataset.record);if(!r||r.kind!=='lot'||!r.allocation)throw Error('Only a copy reserved to a deck can go in that deck’s box.');const l=M.lot(C.state,r.id),d=M.deck(C.state,l.allocation.deckId);await commit({type:'place',lotId:l.id,deckId:d.id,quantity:l.quantity,confirmed:true});C.notice(`${r.card.name} is in ${d.name}’s box.`);};
...
/* A STAND-IN fills a seat while the real card is bought or on its way: the copy goes into the
   box without a reservation, the deck counts it, and the pull sheet asks for it back when a
   real copy is ready. A copy the list does call for is reserved on the way in instead. */
actions['standin-row']=el=>{const d=M.deck(C.state,el.dataset.deck);return quantityAction(el,'Stand in for '+d.name,()=>`<div class="cm-full">${note(`Goes into ${e(d.name)}'s box without a reservation, filling a seat while the real card is bought or on its way. ${e(d.name)} counts it as a stand-in and its pull sheet asks for it back when a real copy is ready. If the list does call for this card, it is reserved on the way in instead.`)}</div>`+f('Box label (optional)','box'),(_,v)=>({type:'place',deckId:d.id,box:v.box,asStandIn:true}));};
