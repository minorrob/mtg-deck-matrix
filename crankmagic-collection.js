...
actions['roster-more']=el=>{const group=el.dataset.group;popAt(el,`${b('Export view','export-view')}${group?`<hr>${b('Manage group','manage-group',{group})}${b('Add planned card','add-group-entry',{group})}${b('Edit planned list','edit-group-entries',{group})}`:''}`);};
C.HELP.collection={title:'Collection',body:`<p>One record per copy lot or requirement: owned, ordered and to-buy copies, with their purpose, print and physical location. Click a row for the card; <strong>Actions</strong> on a row changes its status, deck or location.</p><p>The counts row reads in the order a deck is built. On every deck <strong>Reserved = Owned + Ordered + To buy</strong>; Owned counts reserved copies; the Bench is what you own that no deck has reserved, and Sell / Trade is a flag on Bench copies rather than a count held against a deck. With a filter set, the row counts only what the table shows.</p><p>Six filters are in view; More filters holds subtype, mechanic, flags, offers, mana value and price. Paid and Quantity are cells you click and type in.</p>`};
C.HELP.shop={title:'Shop',body:`<p>The buy list with money on it: every copy a finalized deck still needs, priced, with the cap the rules allow for it. Tick rows and choose <strong>Ordered…</strong> for one order, or <strong>Bought</strong> in a store. Every action is reversible.</p><p><strong>Orders</strong> is one row per order, with what was paid and what has landed; <strong>Ready to add</strong> walks the copies you own into a deck. The strip above the table is the same list in dollars: what finishing costs, per deck.</p>`};
C.HELP.sheet={title:'Spreadsheet',body:`<p>One row per card. <strong>T</strong> is what each deck\u2019s list claims, <strong>A</strong> is what is physically in that deck; Own, Ordered, Bench and To buy run across the row.</p><p>Click a number and type the new one. The library works out what has to move, and asks first when a copy is taken from somewhere.</p>`};
...
function sheet(params){
  ...
  C.main.innerHTML=C.pageHead('Spreadsheet',b('Roster','open-roster')+b('Add a card row','sheet-add')+b('Export CSV','sheet-csv'),'sheet')
...
const cell=(r,k)=>{
  ...
  const body=mixed?'<span class="cm-muted">Various</span>'
    ...
    :e(value(r,k)??'Unknown');
  const standInBadge=k==='placement'&&r.standIn&&r.placement!=='Substitute'?` <span class="cm-badge cm-badge-standin" title="Physically in ${e(M.deck(C.state,r.standInDeckId).name)}, a substitute there until a real copy takes its seat">Substitute in ${e(M.deck(C.state,r.standInDeckId).name)}</span>`:'';
  if(k==='placement'&&(r.option||r.pinned))return (mixed?body:C.pill(body,C.pillKind(r.placement)))+standInBadge+(r.option?` <span class="cm-badge cm-badge-option" title="${e(r.optionWhy||'First candidate to swap out')}">Option</span>`:'')+(r.pinned?' <span class="cm-badge" title="Pinned: kept whatever the Lab or a swap suggests">Pinned</span>':'');
  if(k==='color')return body;
  if(k==='source'&&!mixed)return C.pill(body,C.pillKind(r.source,r.placement));
  if(k==='placement'&&!mixed)return C.pill(body,C.pillKind(r.placement))+standInBadge;
...
actions['roster-columns']=()=>{const shop=C.route().view==='shop',current=shop?shopSelected:selected;form('Choose table columns',`<div class="cm-full cm-columns-grid">${columns.map(([k,l])=>`<label class="cm-checkbox"><input type="checkbox" name="${k}" ${current.includes(k)?'checked':''} ${k==='name'?'disabled':''}>${l}</label>`).join('')}</div><label class="cm-checkbox cm-full"><input type="checkbox" name="__fold" ${foldFor(shop)?'checked':''}>One row per card, whatever the print or deck</label>${s('Rows per page','__pageSize',[['60','60'],['120','120'],['all','All']],C.state.preferences.pageSize||'60')}`,async data=>{const next=['name',...columns.filter(([k])=>k!=='name'&&data[k]).map(([k])=>k)];const values={pageSize:data.__pageSize,[shop?'shopFold':'foldPrints']:!!data.__fold};if(shop){shopSelected=next;values.shopColumns=next;}else{selected=next;values.columns=next;}page=0;await commit({type:'preferences',values});},'Apply columns');};
...
const FILTER_NAMES={q:'Search',type:'Type',subtype:'Subtype',mechanic:'Mechanic',color:'Color',source:'Source',placement:'Allocation',offer:'Bench / Sell / Trade',min:'Min mana value',max:'Max mana value',price:'Max price',group:'Group'};
const activeFilters=()=>Object.entries(filter).filter(([k,v])=>v!==''&&k!=='group');
function chipsHTML(){const on=activeFilters();if(!on.length)return '';const label=(k,v)=>k==='color'?({W:'White',U:'Blue',B:'Black',R:'Red',G:'Green',C:'Colorless'}[v]||v):k==='source'?C.source(v):k==='price'?'$'+v:v;
...
function primary(r){if(r.kind==='fold'){const parts=r.partRows;if(parts.every(p=>p.kind==='need'||p.kind==='lot'&&M.PLANNED.includes(p.source)))return `<button class="v-button primary compact cm-row-buy" data-action="shop-buy" data-record="${e(r.recordId)}">Bought</button><button class="v-button compact cm-row-buy" data-action="shop-order" data-record="${e(r.recordId)}">Ordered</button>`;if(parts.every(p=>p.kind==='lot'&&p.source==='ordered'))return `<button class="v-button primary compact cm-row-buy" data-action="shop-arrive" data-record="${e(r.recordId)}">Arrived</button>`;return '';}
...
actions.replacement=el=>{const d=M.deck(C.state,el.dataset.deck),r=M.slot(C.state,d.id,el.dataset.slot);const main=r.purpose==='main'?r:M.slot(C.state,d.id,r.replaces),options=d.slots.filter(x=>x.replaces===main.id);modal('Options for '+C.state.cards[main.cardId].name,`${note('Suggestions stay outside the committed hundred until you accept a replacement. Released owned copies fulfill compatible needs elsewhere, then move to Bench allocation. Where each one physically is stays recorded.')}${options.map(o=>`<article><h3>${e(C.state.cards[o.cardId].name)}</h3><p>${e(o.purpose)}${o.targetBracket?' · B'+o.targetBracket:''} · ${o.committed?'Reserved option':'Suggestion'}</p>${b('Accept replacement','accept-option',{deck:d.id,slot:o.id})}${b('Remove option','remove-option',{deck:d.id,slot:o.id})}</article>`).join('')||'<p>No linked options yet.</p>'}<div class="cm-actions">${b('Choose a replacement','choose-replacement',{deck:d.id,slot:main.id},true)}${b('Add upgrade / bracket option','choose-option',{deck:d.id,slot:main.id})}</div>`);};
