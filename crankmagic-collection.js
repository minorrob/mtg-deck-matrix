...
/* SIX FIGURES ABOUT WHAT YOU ARE LOOKING AT. They were computed from the whole library
   ...
   are right now: same rows, same question. "In physical deck" is also named for what it counts
...
const STAT_FIGURES=[['reserved','Reserved','plan'],['owned','Owned','have'],['subs','Substitutes','subs'],['physical','In Physical Deck','physical'],['ordered','Ordered','coming'],['toBuy','To Buy','missing'],['watched','Watched','considering']];
function statsHTML(shown,scoped){
  ...
  for(const r of shown){
    ...
    if(r.placement==='In physical deck'||r.placement==='Substitute')t.physical+=r.quantity;
  }
  return `<div class="cm-stats cm-stats-row">${STAT_FIGURES.map(([k,l,g])=>`<div class="cm-stat cm-stat-${g}"><strong>${t[k]}</strong><span>${l}</span></div>`).join('')}</div>`
    +`<p class="cm-status-line cm-stats-scope">${scoped?'Counting the records this view shows. Clear the filters for your whole library.':(()=>{const free=[];if(bench)free.push(`${bench} owned cop${bench===1?'y':'ies'} on the Bench${offered?` (${offered} flagged Sell / Trade)`:''}`);if(orderedFree)free.push(`${orderedFree} ordered cop${orderedFree===1?'y':'ies'}`);return `Reserved ${t.reserved} = ${t.owned} owned + ${t.ordered-orderedFree} ordered + ${t.toBuy} to buy.${free.length?` Reserved by no deck: ${free.join(' and ')}.`:''}`;})()}</p>`;
}
...
function sheet(params){
  ...
  C.main.innerHTML=C.head('The master card roster','Spreadsheet.','One row per card; T is what each deck\u2019s list claims, A is what is physically in the deck. Click a number and type the new one: the library works out what has to move, and asks first when a copy is taken from somewhere.',
    ...
    const body=shown.map(r=>`<tr data-card="${e(r.cardId)}"><th scope="row" class="cm-sheet-name"><button type="button" class="cm-text-button cm-card-name" data-action="card" data-card="${e(r.cardId)}" title="${e(r.card.typeLine||'')}">${e(r.card.name)}</button>${r.extra?' <span class="cm-badge">New row</span>':''}</th>`
      ...
      +decks.map((d,i)=>{const p=r.perDeck[d.id],short=d.status==='final'&&p.t>p.a,pend=p.a-p.boxed;
        ...
        return live(r,'t',d.id,p.t,{cls:alt(i)+(short?' is-short':''),mark:marks,label:`${d.name}: copies of ${r.card.name} in the list`,title:short?`${d.name} lists ${p.t}, ${p.a} covered`:''})
          +(d.status==='final'?live(r,'boxed',d.id,p.boxed+p.sub,{cls:alt(i)+(p.boxed&&p.boxed>=p.t?' is-done':''),mark:(pend>0?`<sup class="cm-sheet-pend" title="${pend} more reserved to ${e(d.name)}, owned and still to pull">+${pend}</sup>`:'')+(p.sub?`<sup class="cm-sheet-sub" title="${p.sub} cop${p.sub===1?'y':'ies'} of ${e(r.card.name)} standing in as ${p.sub===1?'a substitute':'substitutes'} in ${e(d.name)}: physically there, not called for by its list">ˢ${p.sub}</sup>`:''),label:`${d.name}: copies of ${r.card.name} physically in the deck`})
...
const cell=(r,k)=>{
  ...
  const body=mixed?'<span class="cm-muted">Various</span>'
    ...
    :e(value(r,k)??'Unknown');
  const standInBadge=k==='placement'&&r.standIn&&r.placement!=='Substitute'?` <span class="cm-badge cm-badge-standin" title="Physically in ${e(M.deck(C.state,r.standInDeckId).name)}, a substitute there until it is pulled">Substitute in ${e(M.deck(C.state,r.standInDeckId).name)}</span>`:'';
...
function primary(r){if(r.kind==='fold'){const parts=r.partRows;if(parts.every(p=>p.kind==='need'||p.kind==='lot'&&M.PLANNED.includes(p.source)))return `<button class="v-button primary compact cm-row-buy" data-action="shop-buy" data-record="${e(r.recordId)}">Bought</button><button class="v-button compact cm-row-buy" data-action="shop-order" data-record="${e(r.recordId)}">Ordered</button>`;if(parts.every(p=>p.kind==='lot'&&p.source==='ordered'))return `<button class="v-button primary compact cm-row-buy" data-action="shop-arrive" data-record="${e(r.recordId)}">Arrived</button>`;return '';}
  ...
  if(r.kind==='lot'&&r.source==='owned'&&r.allocation&&r.placement!=='In physical deck'){const d=M.deck(C.state,r.allocation.deckId);return `<button class="v-button primary compact cm-row-buy" data-action="place-now" data-record="${e(r.recordId)}" title="Record that this copy is in ${e(d.name)}’s physical deck">Put in ${e(shortDeck(d))}</button>`;}
