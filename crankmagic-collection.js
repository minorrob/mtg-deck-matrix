...
const SHOP_COLUMNS=[['name','Card'],['color','Color'],['type','Type'],['price','Price'],['paid','Paid']];
...
const cell=(r,k)=>{
  ...
  const body=mixed?'<span class="cm-muted">Various</span>'
    ...
    :k==='price'?(tight&&!(r.card.price>0)?'—':C.money(r.card.price))
  ...
  if(k==='source')return `<span class="cm-badge">${body}</span>`;
