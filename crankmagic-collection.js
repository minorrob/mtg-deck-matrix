...
function tabletop(params,shop=false){
  ...
  const draw=()=>{
    ...
    /* A BAND YOU EMPTIED IS NOT A BAND THAT VANISHED (Rob, 15 September). Pull the last card out
       of the open pile and it leaves the model; closing the drawer on that sends the reader back to
       find a pile that is no longer drawn. A group id still naming the current grouping is kept, and
       the mat draws it empty; a grouping change already clears it in onGroupBy, so what is dropped
...
function sheet(params){
  ...
  C.main.innerHTML=cardsHead(params,'library','sheet')
   +`<div class="cm-toolbar"><label class="cm-search">Search cards<input id="cm-sheet-query" value="${e(sheetQ)}" placeholder="Card name or type"></label>${s('Show','sheetOnly',SHEET_SHOW,sheetOnly)}${s('Deck','sheetDeck',[['','All decks'],...decks.map(d=>[d.id,d.name])],sheetDeck)}${b('Back to Play Space','open-tabletop')}</div>`
