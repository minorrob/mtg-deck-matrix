...
/* SIX FIGURES ABOUT WHAT YOU ARE LOOKING AT. They were computed from the whole library
   ...
   are right now: same rows, same question. "In deck" is also named for what it counts --
   copies whose physical box is a deck -- rather than for cards a deck list contains. */
const STAT_FIGURES=[['owned','owned copies'],['ordered','ordered'],['wanted','wanted'],['toBuy','to buy'],['inDeck','in a deck box'],['sellTrade','Sell / Trade']];
function statsHTML(shown,scoped){
  ...
  for(const r of shown){
    ...
    t.owned+=r.quantity;
    if(r.placement==='In deck')t.inDeck+=r.quantity;
...
function show(params,shop){selected=selected||C.state.preferences.columns||defaults;const tight=compactShop(shop);
