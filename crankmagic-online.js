(globalThis.CrankFeatures ||= []).push(function(C){
  ...
  C.views.game=async()=>{
    ...
    if(local){
      ...
      const onMessage=event=>{if(event.origin!==location.origin||event.source!==frame.contentWindow)return;if(event.data?.type==='crankmagic-live'){C.main.classList.add('cm-playing');C.main.classList.remove('cm-setting-up');}if(event.data?.type==='crankmagic-mode'&&event.data.mode==='setup'){C.main.classList.remove('cm-playing');C.main.classList.add('cm-setting-up');}if(event.data?.type==='crankmagic-canvas-size'&&Number.isFinite(event.data.height)){frame.style.height=Math.max(300,Math.min(25000,event.data.height+2))+'px';}if(event.data?.type==='crankmagic-focus')frame.classList.toggle('cm-game-expanded',!!event.data.open);if(event.data?.type==='crankmagic-exit')location.hash='decks';};
