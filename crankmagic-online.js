(globalThis.CrankFeatures ||= []).push(function(C){
  ...
  C.views.game=async()=>{
    ...
    if(local){
      ...
      const onMessage=event=>{if(event.origin!==location.origin||event.source!==frame.contentWindow)return;if(event.data?.type==='crankmagic-live')C.main.classList.add('cm-playing');if(event.data?.type==='crankmagic-focus')frame.classList.toggle('cm-game-expanded',!!event.data.open);if(event.data?.type==='crankmagic-exit')location.hash='decks';};
      window.addEventListener('message',onMessage);return()=>{window.removeEventListener('message',onMessage);layout.classList.remove('cm-play-collapsed');C.main.classList.remove('cm-playing');};
    }
