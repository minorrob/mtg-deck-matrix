...
const upgrade=card('Shiny Upgrade');
const pool=new Map([leader,...filler,mountain,upgrade].map(c=>[L.fold(c.name),c]));
...
eq(L.names(doc()).length,43);// leader, 40 filler, Mountain, Shiny Upgrade, and nothing counted twice
...
// The segments on the live file: D3 owns 72, 70 of them in its box, so 2 are to pull.
{const d3=saved.state.decks.find(d=>/^D3/.test(d.name)),r=M.readiness(saved.state,d3);eq(r.inBox,70);eq(r.pullFromBench+r.pullFromOtherBox,2);eq(r.ordered,4);eq(r.toBuy,24);eq(r.remove,0);ok(r.costToFinish>0);}
