...
{const before=M.counters(state);run('acquire',{lot:{id:'want1',cardId:pool[2].id,quantity:2,source:'watching'}});const after=M.counters(state);eq(after.watching,before.watching+2);eq(after.toBuy,before.toBuy);eq(after.owned,before.owned);
 ...
 run('source',{lotId:'want1',source:'owned',quantity:2});const owned=state.lots.find(l=>l.id==='want1');eq(owned.source,'owned');eq(owned.location.kind,'bench');eq(M.counters(state).watching,before.watching);eq(M.counters(state).owned,before.owned+2);
 run('acquire',{lot:{id:'want2',cardId:pool[3].id,quantity:1,source:'watching'}});eq(M.counters(state).watching,before.watching+1);run('removePending',{lotId:'want2',quantity:1});ok(!state.lots.some(l=>l.id==='want2'));eq(M.counters(state).watching,before.watching);}
