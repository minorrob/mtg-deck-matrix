...
(function(root,factory){const classifier=typeof module==='object'&&module.exports?require('./card-classify.js'):root.MtgCardClassify;const payload=typeof module==='object'&&module.exports?require('./graph-payload.js'):root.CrankGraphPayload;const api=factory(classifier,payload);if(typeof module==='object'&&module.exports)module.exports=api;if(root)root.CrankCatalog=api;})(typeof globalThis!=='undefined'?globalThis:this,function(Classify,Payload){
  ...
  async function create(options){const byName=new Map(),byAlias=new Map(),byId=new Map(),byOracle=new Map();let graph=null,universeDate='',rankDate='',priceDate='',graphDate='',graphLoading=null,playedLoading=null;const fetcher=options.fetchImpl||fetch;
    ...
    /* THE LIBRARY REFERENCES THE RECORD. A saved library card whose name the shipped record
       set carries contributes its identity and nothing else -- oracle id, printed name, when
       it was saved -- and the facts stay the record's, so a vocabulary or price change in
       data/cards.json reaches every library and a stale copy can never shadow it. A card the
       record set does not carry is kept whole: the library is the record's cache for it. */
    const OVERLAY=['oracleId','scryfallId','flavorName','flavorNames','updatedAt'];
    function overlay(raw){const prior=raw&&raw.name?named(raw.name):null;if(!(prior&&prior.shipped))return add(raw);const keep={};for(const k of OVERLAY)if(raw[k]!==undefined&&raw[k]!==''&&!(Array.isArray(raw[k])&&!raw[k].length))keep[k]=raw[k];if(prior.oracleId)delete keep.oracleId;return add({...prior,...keep,flavorNames:[...new Set([...(prior.flavorNames||[]),...(raw.flavorNames||[]),...(raw.flavorName?[raw.flavorName]:[])])],shipped:true});}
    ...
    const expect=typeof options.urls.expect==='function'?options.urls.expect:(json)=>json;const settled=await Promise.allSettled([load(options.urls.universe).then(j=>expect(j,'universe')),load(options.urls.cards).then(j=>expect(j,'cards')),load(options.urls.facts).then(j=>expect(j,'facts')),options.urls.ranks?load(options.urls.ranks).then(j=>expect(j,'ranks')):Promise.resolve(null),options.urls.flavorNames?load(options.urls.flavorNames).then(j=>expect(j,'flavorNames')):Promise.resolve(null)]);
    ...
    if(settled[1].status==='fulfilled'){priceDate=settled[1].value.generatedAt||'';for(const c of settled[1].value.cards)add({...c,shipped:true});}
    if(settled[2].status==='fulfilled')for(const [name,c] of Object.entries(settled[2].value.cards))add({name,...c});
    for(const c of Object.values(options.savedCards||{}))overlay(c);
